...
1. Selecting a label $y^{(i)}$ from $P(y)$. If our model is predicting `red or green or blue`, $P(y)$ is likely a multinomial distribution. (If our observed label counts are `{'red': 20, 'green': 50, blue: 30}`, we retrodictively believe this multinomial distribution to have had a parameter vector near $\pi = [.2, .5, .3]$.)
...
We now have the numerator in hand — $P(y)$, and three distinct conditional distributions $P(x\vert y=\text{red}), P(x\vert y=\text{green})$, and $P(x\vert y=\text{blue})$; what about the denominator?

#### Conditional probability and marginalization
The axiom of conditional probability allows us to write $P(B\vert A)P(A) = P(B, A)$, i.e. the *joint probability* of $B$ and $A$. This is a simple manipulation of probability distributions. As such, we can rewrite Bayes' theorem in its more compact form.
...
Marginalization took me a while to understand; here's a simple example. Imagine we have the following joint probability distribution out of which we'd like to marginalize $A$.
...
2. "Collapse" the remaining columns — in this case, that of $B$.
...
The resulting probability distribution is over a single value — it yields a scalar. This scalar *normalizes* the respective numerator terms, such that:
...
- *Conversely, marginalization is a much more general operation performed on a probability distribution.*
  - It yields a scalar only when the remaining variables(s) are homogeneous, i.e. each remaining columns contains a single distinct value.
...
If we neglect to compute $P(x)$, i.e. if we don't normalize our joint distributions $P(x, y = k)$, we'll be left with an invalid probability distribution $\tilde{P}(y\vert x)$ whereby the values do not sum to 1. This distribution might look like `P(y|x) = {'red': .00047, 'green': .0011, 'blue': .0000853}`. *If our goal is to simply compute the most likely label — taking the argmax of the unnormalized distribution works just fine.* This follows trivially from our pool ring:
